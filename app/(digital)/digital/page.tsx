...
import { DIGITAL_CATEGORIES, DIGITAL_PIECES, type DigitalCategory } from "../../lib/digital-gallery";

export default function DigitalGalleryPage() {
  const [active, setActive] = useState<DigitalCategory | "all">("all");

  const items = useMemo(() => {
    if (active === "all") return DIGITAL_PIECES.filter((p) => p.category !== "studies");
    if (active === "studies") return DIGITAL_PIECES.filter((p) => p.category === "studies");
    return DIGITAL_PIECES.filter((p) => p.category === active);
  }, [active]);
